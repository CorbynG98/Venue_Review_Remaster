...
const getUsernameEmailById = (user_id) => {
    return new Promise((resolve, reject) => {
        (0, db_1.getPool)().query('SELECT username, email FROM User WHERE user_id = ?', user_id, (err, result) => {
            if (err)
                return reject(err);
...
const getUserByEmail = (email) => {
    return new Promise((resolve, reject) => {
        (0, db_1.getPool)().query('SELECT user_id, password, given_name, family_name, profile_photo_filename FROM User WHERE email = ? LIMIT 1', email, (err, result) => {
            if (err)
                return reject(err);
            if (result == '' || result == null || result.length == 0)
                return resolve(null);
...
const getUserByUsername = (username) => {
    return new Promise((resolve, reject) => {
        (0, db_1.getPool)().query('SELECT user_id, password, given_name, family_name, profile_photo_filename FROM User WHERE username = ? LIMIT 1', username, (err, result) => {
            if (err)
                return reject(err);
            if (result == '' || result == null || result.length == 0)
                return resolve(null);
