...
const checkReviewer = (user_id, venue_id) => {
    return new Promise((resolve, reject) => {
      ...
      WHERE
      v.venue_id = ?
      AND v.admin_id != ?`, [venue_id, user_id, venue_id, user_id], (err, result) => {
            if (err)
                return reject(err);
...
const createReview = (values) => {
    return new Promise((resolve, reject) => {
        (0, db_1.getPool)().query('INSERT INTO Review (review_id, reviewed_venue_id, review_body, star_rating, cost_rating, time_posted, review_author_id) VALUES (?, ?, ?, ?, ?, ?, ?)', values, (err) => {
            if (err)
                return reject(err);
            resolve();
        });
